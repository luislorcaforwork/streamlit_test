...
def get_drive_folder(drive_service, folder_name):
    ...
    if not folder_list:
        folder = drive_service.CreateFile({'title': folder_name, 'mimeType': 'application/vnd.google-apps.folder'})
        folder.Upload()
...
def gem_app():
    ...
    DRIVE_FOLDER_NAME = st.sidebar.text_input("Google Drive Folder Name", value=DRIVE_FOLDER_NAME)
    ...
    if upload_option == 'Tomar una foto con la cámara':
        ...
        uploaded_file = st.file_uploader("Sube una imagen de la galería", type=["png", "jpg", "jpeg"])
    ...
    if image_data:
        ...
        if drive:
            ...
            if drive_folder_id:
                ...
                st.error(f"No se pudo encontrar o crear la carpeta de Google Drive '{DRIVE_FOLDER_NAME}'.")
        else:
            st.warning("La autenticación de Google Drive falló. Por favor, revisa tus credenciales.")
